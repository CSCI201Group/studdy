...
const Login = () => {
  ...
  React.useEffect(() => {
    //only happens once valid changes, since it takes a bit for valid to change

    if (valid === true) {
      //if false then set correct error message
      navigate("/Swipe", { state: { username: username } });
      setMessage("");
    }
  }, [valid]);
  ...
  return (
    <div>
      ...
      <div id="box">
        ...
        <br />
        <div id="DNE">
          <Link to={"/GuestReg"}>Continue as guest!</Link>
        </div>
      </div>
    </div>
  );
